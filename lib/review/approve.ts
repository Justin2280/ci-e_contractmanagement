...
export const ApprovePayloadSchema = z.object({
  ...
  contract: z.object({
    ...
    indexatieWijze: z.enum(indexatieWijze.enumValues).optional(),
    indexatieAanvraagMoment: z.string().regex(/^\d{2}-\d{2}$/).nullable().optional(),
    indexatieKwartaal: z.number().int().min(1).max(4).nullable().optional(),
...
export async function approveExtraction(payload: ApprovePayload, userId: string) {
  ...
  return db.transaction(async (tx) => {
    ...
    const contractValues = {
      ...
      indexatieWijze: p.contract.indexatieWijze ?? "vooraf",
      indexatieAanvraagMoment: p.contract.indexatieAanvraagMoment ?? null,
      indexatieKwartaal: p.contract.indexatieKwartaal ?? null,
